...
def get_app_details(package_id):
    ...
    try:
        print('Fetching Details from Play Store: ', package_id)
        det = app(package_id)
...
def app_search(app_id):
    ...
    if not settings.APPMONSTA_API:
        return det
    print('Fetching Details from AppMonsta: %s', app_id)
    ...
    try:
        ...
        print('Unable to get app details')
        return det


def app_category(pkg: str):
    return get_app_details(pkg).get("genre")
